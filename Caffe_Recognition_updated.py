...
img1="/Users/resitkadir/Downloads/BS_Face/Caffe-Recognition-app/items/"
tick = cv2.imread(img1+"tick.png")
#"/Users/resitkadir/Download/BS_Face/Caffe-Recognition-app/items/tick.png")
tick = cv2.resize(tick, (40, 40))

cross = cv2.imread(img1+"cross.png")
#"/Users/resitkadir/Download/BS_Face/Caffe-Recognition-app/items/cross.png")
cross = cv2.resize(cross, (40, 40))


class DectectFace:
    def __init__(self,model,name,age,gender):
        self.face_dir = '/Users/resitkadir/Download/BS_Face/Caffe-Recognition-app/Data/face_data'
        self.face_name = name
        self.face_age = age
        ...
        self.net = cv2.dnn.readNetFromCaffe('/Users/resitkadir/Download/BS_Face/Caffe-Recognition-app/Models/face_deploy.prototxt.txt','/Users/resitkadir/Desktop/BS_Face/Caffe-Recognition-app/Models/face_net.caffemodel')
    ...
    def process_image(self,inImg):
        ...
        if self.count_captures < NUM_TRAINING:
            ...
            for i in range(0,detections.shape[2]):
                ...
                cv2.putText(frame,text,(20,40),cv2.FONT_HERSHEY_SIMPLEX,0.45,(255,255,255),2)
            ...
            mylines = []
            f = open("/Users/resitkadir/Download/BS_Face/Caffe-Recognition-app/Data/face_data/age_gender.txt","a")
...
class RecogLBPHFaces:
    def __init__(self):
        ...
        self.face_ages = []
        self.face_genders = []
        self.age_net = cv2.dnn.readNetFromCaffe('/Users/resitkadir/Download/BS_Face/Caffe-Recognition-app/Models/age_deploy.prototxt','/Users/resitkadir/Download/BS_Face/Caffe-Recognition-app/Models/age_net.caffemodel')
        self.gender_net = cv2.dnn.readNetFromCaffe('/Users/resitkadir/Download/BS_Face/Caffe-Recognition-app/Models/gender_deploy.prototxt','/Users/resitkadir/Download/BS_Face/Caffe-Recognition-app/Models/gender_net.caffemodel')
        self.ap = argparse.ArgumentParser()
        self.ap.add_argument("-c","--confidence",type=float,default=0.5,
                             help="minimum probability to filter weak detections")
        self.args = vars(self.ap.parse_args())
        self.net = cv2.dnn.readNetFromCaffe('/Users/resitkadir/Download/BS_Face/Caffe-Recognition-app/Models/face_deploy.prototxt.txt','/Users/resitkadir/Download/BS_Face/Caffe-Recognition-app/Models/face_net.caffemodel')
    ...
    def load_trained_data(self):
        ...
        for (subdirs,dirs,files) in os.walk(self.face_dir):
            for subdir in dirs:
                ...
                key += 1
        self.names = names
        self.model.read('/Users/resitkadir/Download/BS_Face/Caffe-Recognition-app/Models/LBPH_trained_data.xml')
    ...
    def process_image(self,inImg):
        ...
        for i in range(0,detections.shape[2]):
            ...
            if confidence[1] < 100:
                ...
                detage = []
                detgender = []
                with open("/Users/resitkadir/Download/BS_Face/Caffe-Recognition-app/Data/face_data/age_gender.txt", "r") as file:
                    for item in file:
                        data = item.split()
                        if data[0] == person:
                            detage = data[1]
                ...
                #cv2.putText(frame, 'Info: |%s- |%s- |%s- |%s|' % ("name","gender", "age", "cal"), (10,20),cv2.FONT_HERSHEY_SIMPLEX,0.45,(0,0,0),2)
                cv2.putText(frame, 'Info: |%s-| |%s-| |%s-| |%s|' % (person,detgender, detage,cal), (20,40),cv2.FONT_HERSHEY_SIMPLEX,0.45,(0,255,0),2)
                ...
                cv2.putText(frame, 'Info: %s- %s' % (person, overlay_text),(400,400),cv2.FONT_HERSHEY_SIMPLEX,0.45,(0,0,255),2)
                #cv2.putText(frame,person,(startX,y),cv2.FONT_HERSHEY_SIMPLEX,0.45,(0,0,255),2)
